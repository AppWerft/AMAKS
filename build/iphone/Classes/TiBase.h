...
// in simulator we redefine to format for AMAKS Developer console
...
{\
	NSString * exceptionName = [@"org.amaks." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.amaks";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UIAMAKSNativeItemNone = -1, 
	UIAMAKSNativeItemSpinner = -2,
	UIAMAKSNativeItemProgressBar = -3,
	
	UIAMAKSNativeItemSlider = -4,
	UIAMAKSNativeItemSwitch = -5,
	UIAMAKSNativeItemMultiButton = -6,
	UIAMAKSNativeItemSegmented = -7,
	
	UIAMAKSNativeItemTextView = -8,
	UIAMAKSNativeItemTextField = -9,
	UIAMAKSNativeItemSearchBar = -10,
	
	UIAMAKSNativeItemPicker = -11,
	UIAMAKSNativeItemDatePicker = -12,
	
	UIAMAKSNativeItemInfoLight = -13,
	UIAMAKSNativeItemInfoDark = -14,
	
	UIAMAKSNativeItemDisclosure = -15,
	
	UIAMAKSNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all AMAKS instances of
